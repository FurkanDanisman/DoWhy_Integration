"""
Do-calculus rewrite utilities for causal probability expressions.

This module provides symbolic transformation rules and diagnostics used to
manipulate causal probability expressions under a given causal graph.
"""
...
__all__ = ["CausalExpr", "has_descendant_in_set", "is_d_separated"]

logger = logging.getLogger(__name__)


def is_d_separated(graph: nx.DiGraph, start, end, conditioned_set: Iterable) -> bool:
    """
    ...
    :param graph: Directed acyclic graph for d-separation testing.
    :param start: Source node.
    :param end: Target node.
    :param conditioned_set: Iterable of conditioning nodes.
    :returns: True if d-separated, otherwise False.
    """
...
def has_descendant_in_set(
    graph: nx.DiGraph, node, conditioned_set: Iterable, visited: Optional[Set] = None
) -> bool:
    """
    Depth-first search to check whether a node has descendants in a set.

    :param graph: Directed graph to traverse.
    :param node: Node to start from.
    :param conditioned_set: Iterable of nodes to check against.
    :param visited: Set of visited nodes (for recursion).
    :returns: True if any descendant is in conditioned_set, otherwise False.
    """
    if visited is None:
        visited = set()
    if node in visited:
...
class CausalExpr:
    ...
    The graph is derived from the provided causal structure and used for
    d-separation checks required by do-calculus rules.
    """

    def __init__(self, expression, causal_structure: Optional[Dict] = None):
        """
        Initialize a causal expression and build its graph.

        :param expression: Symbolic expression (e.g., CausalProbability).
        :param causal_structure: Optional dict mapping parent -> children.
        """
    ...
    def copy(self):
        ...
        :returns: New CausalExpr instance with the same structure.
        """
        return CausalExpr(
            self.expression.copy() if hasattr(self.expression, 'copy') else self.expression,
            self.causal_structure,
        )

    def _make_acyclic_copy(self, graph: Optional[nx.DiGraph] = None) -> nx.DiGraph:
        """
        ...
        :param graph: Optional graph to process; defaults to self.graph.
        :returns: A (best-effort) acyclic graph copy.
        """
        if graph is None:
            graph = self.graph
    ...
    def _custom_d_separation(self, X, Y, Z, graph: Optional[nx.DiGraph] = None) -> bool:
        """
        Wrapper around is_d_separated with string normalization.

        :param X: Source node.
        :param Y: Target node.
        :param Z: Conditioning set.
        :param graph: Optional graph to use; defaults to self.graph.
        :returns: True if d-separated, otherwise False.
        """
        if graph is None:
            graph = self.graph
    ...
    def _is_d_separated(self, X, Y, Z) -> bool:
        """
    ...
    def _create_intervention_graph(self, do_vars: Iterable) -> nx.DiGraph:
        """
        Create a modified graph where incoming edges to intervention variables are removed.
        This is G_X for do(X).

        :param do_vars: Iterable of intervention variables.
        :returns: Modified DiGraph with incoming edges removed for do_vars.
        """
    ...
    def apply_rule_1_all(self):
        ...
        :returns: List of CausalProbability expressions after Rule 1 rewrites.
        """
        outs = []
        seen = set()
    ...
    def apply_rule_2_all(self):
        """
        Enumerate all one-step Rule 2 (action/observation exchange) rewrites:

        P(Y | do(X), do(Z), W) = P(Y | do(X), Z, W)
        if Y ⟂ Z | X, W in G_{\bar X, underline{Z}}

        Returns a list of all distinct expressions obtainable by replacing
        exactly one do(Z) with observation Z when the criterion holds.

        :returns: List of CausalProbability expressions after Rule 2 rewrites.
        """
    ...
    def apply_rule_3_all(self):
        ...
        :returns: List of CausalProbability expressions after Rule 3 rewrites.
        """
        outs = []
        seen = set()
    ...
    def suggest_fix(self):
        ...
        for z in obs_vars:
            if self.graph.has_edge(z, str(Y)):
                is_mediator = any(nx.has_path(self.graph, x, z) for x in obs_vars if x != z)
                if is_mediator:
                    suggestions.append(
