"""
Breadth-first do-calculus proof search utilities.

This module provides a BFS-based proof search over do-calculus rewrite rules.
...
class CausalProofFinder:
    ...
    def __init__(self, causal_structure=None, max_depth: int = 10):
        """
        Initialize a proof finder for do-calculus rewrites.

        :param causal_structure: Optional dict mapping parent -> children.
        :param max_depth: Maximum BFS depth for proof search.
        """
    ...
    def _are_expressions_equivalent(self, expr1, expr2) -> bool:
        """
    ...
    def _as_subtraction_pair(self, expr) -> Optional[Tuple[sp.Expr, sp.Expr]]:
        """
        If expr is of the form A - B, return (A, B).
        SymPy represents subtraction as Add(A, -B).

        :param expr: SymPy expression to inspect.
        :returns: Tuple (A, B) if expr is A - B, else None.
        """
    ...
    def _state_key(self, expr) -> str:
        """
        Normalize an expression into a stable string key.

        :param expr: CausalProbability or SymPy expression.
    ...
    def _do_calculus_successors(self, expr) -> List[Tuple[str, CausalProbability]]:
        """
        Return list of (rule_label, next_expr) for ALL one-step do-calculus rewrites.

        :param expr: Current CausalProbability expression.
        :returns: List of (rule_label, next_expr) successors.
        """
    ...
    def find_proof(self, start_expr, target_expr) -> Optional[List]:
        """
        ...
        :param start_expr: Starting expression.
        :param target_expr: Target expression.
        :returns: Proof path list, [] if already equivalent, or None if not found.
        """
        ...
        raise TypeError(
            "Unsupported expression type: expected CausalProbability or A-B of CausalProbability"
        )

    def _find_proof_single(
        self, start_expr, target_expr
    ) -> Optional[List[Tuple[str, CausalProbability]]]:
        """
        BFS proof search for single CausalProbability expressions.

        :param start_expr: Starting CausalProbability.
        :param target_expr: Target CausalProbability.
        :returns: Proof path list, [] if already equivalent, or None if not found.
        """
        ...
        queue: Deque[Tuple[CausalProbability, List[Tuple[str, CausalProbability]]]] = deque(
            [(start_expr, [])]
        )
        visited = {start_key}
        ...
        while queue:
            ...
            if (
                self._are_expressions_equivalent(cur, target_expr)
                or self._state_key(cur) == target_key
            ):
    ...
    def explore_all_equivalent_expressions(
        self, start_expr, max_depth: Optional[int] = None
    ) -> Dict[str, Tuple[CausalProbability, str]]:
        """
        ...
        :param start_expr: Starting CausalProbability expression.
        :param max_depth: Optional maximum depth; defaults to self.max_depth.
        :returns: Mapping of state key to (expression, last rule label).
        """
